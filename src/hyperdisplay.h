...
class hyperdisplay : public Print{
    ...
    // Methods
        ...
        virtual void xline(uint16_t x0, uint16_t y0, uint16_t Len, color_t color[], uint16_t colorCycleLength, uint16_t width = 1); //default implement with many pixels so that user CAN just add a way to write to a pixel,  but highly reccommend optimizing
        ...
        virtual void fillFromArray(uint16_t x, uint16_t y, uint16_t size, color_t data[]); //how much can you overwrite?? the whole function definition? like what if i want to add another param
           
    //does the derived class need to keep virtual keyword? usually no, right? unless 64x64 draws different than 50x50 for instance right
